...
|---|---|---|---|
| v2.9.1 | 2026-09-08 (KST) | Final | CC BY-NC-SA 4.0 (see [License](#license)) |
...
**A consultant or facilitator delivering an engagement.** Read [consultant-workbook.md](./consultant-workbook.md): the 30-day initial engagement on one priority workflow for a named business owner, with the delivery cadence, twenty-six working records keyed to the steps and gates, measurement rules that count all human effort once and a fictional teaching case that fails expansion on quality while passing on effort. It changes no step, gate, tier, role or principle.
...
**A professional or team delegating work to AI agents.** Read [take-out-delivery-and-vending-machines.md](./take-out-delivery-and-vending-machines.md): the operating companion for preparing work, authorizing execution and maintaining bounded services at distributed points of use. Includes a service ticket, a service receipt, recovery guidance and fictional examples. It changes no step, gate, tier, role or principle.
...
| [primer/](./primer/README.md) | The Practice Primer in 24 derived parts with a reading index, so the text can be read in the browser. Derived from the single file above and verified to rebuild it byte for byte | You want to read the Primer rather than download it |
...
| [consultant-workbook.md](./consultant-workbook.md) | Standalone delivery instrument: the 30-day initial engagement on one priority workflow, with a consultant operating brief, the delivery cadence, the method and the Feast mapped to twenty-six working records (W01 to W24 with W05A and W24A), the canonical full-day agenda with the evidence each session produces, a measurement protocol, a fictional supplier-dossier teaching case and the delivery acceptance check. Sources are cited by name and versions pinned once in its resource register; W10 carries both limbs of Informed Intent as stated in DOCTRINE.md | You are delivering the method to a Client as a bounded engagement, or training the consultants who will |
| [take-out-delivery-and-vending-machines.md](./take-out-delivery-and-vending-machines.md) | Standalone operating companion for AI agents: the three service patterns (take-out, delivery and vending machines), a service ticket and service receipt keyed to Informed Intent, the four-property test applied to a live system, stop, reconcile and hand-back procedure and a fictional worked order carrying an investigation into an authorized send batch and on to the same service at three branches. It changes no step, gate, tier, role or principle | You are commissioning an agent run or maintaining a reusable agent service under the existing method |
...
> Paik, Son-U Michael. *Slow AI Kitchen: Method and Workshop Guide*, v2.9.1. GRC Solutions Korea, 2026. https://github.com/rolldabones/slow-ai-kitchen
...
Current version: **v2.9.1 (2026-09-08)**. A style correction across nine module files. Twenty-three serial commas are removed from the author's own prose, including the three lawful outcomes of Gate 7, which read "confirm, refuse or amend and confirm" in `consultant-workbook.md`, `service-record.md` and `newtons-kitchen.md` and carried the comma in all three. The v2.8.0 claim that house style was applied in the Consultant Workbook conversion is struck in the changelog. Quotations, the license text, shipped changelog entries and EU AI Act pinpoint citations are excluded and unchanged, and the Primer and its derived parts are unchanged byte for byte. No step, gate, tier, role or principle changes. Prior at v2.9.0: Take-Out, Delivery & Vending Machines joins the repository as a standalone operating companion for AI agents: the three service patterns, a service ticket and service receipt keyed to Informed Intent, Newton's four-property test applied to the executing system, the stop, reconcile and hand-back procedure and a fictional worked order that refuses confirmation of an incomplete batch. It changes no step, gate, tier, role or principle. Prior at v2.8.0: The Consultant Workbook joins the repository as a standalone delivery instrument: the 30-day initial engagement on one priority workflow, twenty-six working records keyed to the steps and gates, measurement rules and a fictional teaching case. Sources are cited by name with one dated currency table, W10 carries both limbs of Informed Intent as stated in DOCTRINE.md v1.1.1 and the Service Record is wired into Step 9, W05, W13, the worked case and the acceptance check. Prior at v2.7.0: the Service Record joins the repository as a standalone module, the one-page close-out of a single service, written by the named owner within a day of release, required for Tier 3 work, recommended for Tier 2 and optional for Tier 1; Step 9 names it, the Tier Application Map carries it and the Definition of Done for the Method Itself gains one test for it. Prior at v2.6.2: citation infrastructure, CITATION.cff, a How to Cite block pointing at it, the doctrine citation line and the company name corrected to BABL AI in the About the author section. Prior at v2.6.1: correction to v2.6.0, one of the 23 parts published that morning was 311,517 bytes and GitHub declined to render it, so the release did not deliver what it claimed. The parts are regenerated under a size ceiling and there are now 24. Prior at v2.6.0: the Practice Primer published in parts for reading under [primer/](primer/README.md), generated from the single file, which remains the file of record and is unchanged byte for byte. Prior at v2.5.1: Karpathy's Kitchen closing line aligned to DOCTRINE.md, "identifiable human principal" corrected to "named human owner" under the doctrine delta rule; logged at v2.4.0 for the weekly sweep and executed in this release round. Prior at v2.5.0: Coetzee's Kitchen joined as a standalone add-on module, the epistemics of the human-AI working session read through Coetzee's 2003 Nobel lecture, introducing Return to Source and Cognitive Provenance as operating disciplines and supplying the socio-technical warrant for Gate 3, with the How to Cite correction. No change to the twelve steps, the ten gates, the three tiers, the workshop design, the Your Restaurant addendum, the Primer, the three derivation modules or the Service Record.
